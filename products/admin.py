...
class AdminImageWidget(forms.ClearableFileInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        if value and hasattr(value, "url"):
            ...
            html.append(
                format_html(
                    '  <img id="{0}" src="{1}" alt="Preview" style="max-width: 100%; max-height: 140px; border-radius: 8px; object-fit: contain; margin-bottom: 12px; box-shadow: var(--admin-shadow-sm); border: 1px solid var(--admin-border-light);">'
                    '  <div id="{2}" style="display: none;"><i class="fas fa-image" style="font-size: 3rem; color: var(--admin-text-muted); margin-bottom: 16px;"></i></div>'
                    '  <input type="checkbox" name="{4}-clear" id="{3}" class="image-clear-checkbox" style="position: absolute; opacity: 0; width: 1px; height: 1px; overflow: hidden;" onchange="toggleImageRemoval(this, \'{0}\', \'{2}\')">'
                    '  <label class="image-remove-btn" for="{3}" style="color: #dc3545; font-size: 0.9rem; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; margin-bottom: 12px; user-select: none;">'
                    '    <span class="remove-text" style="transition: all 0.2s ease;">Remove</span>'
                    '  </label>'
                    '  <label for="{5}" class="btn-change-image" style="background: var(--admin-gradient-primary); color: #ffffff; padding: 10px 20px; border-radius: var(--admin-radius-sm); font-weight: 600; font-size: 0.85rem; cursor: pointer; display: inline-block; transition: all 0.2s ease; box-shadow: 0 4px 12px var(--admin-accent-glow); text-transform: uppercase; margin-bottom: 8px;">'
                    '    Change Image'
                    '  </label>',
                    img_id, value.url, placeholder_id, clear_checkbox_id, name, input_id
                )
            )
        else:
            ...
            html.append(
                format_html(
                    '  <img id="{0}" src="" alt="Preview" style="max-width: 100%; max-height: 140px; border-radius: 8px; object-fit: contain; margin-bottom: 12px; box-shadow: var(--admin-shadow-sm); border: 1px solid var(--admin-border-light); display: none;">'
                    '  <div id="{1}"><i class="fas fa-image" style="font-size: 3rem; color: var(--admin-text-muted); margin-bottom: 16px;"></i></div>'
                    '  <input type="checkbox" name="{3}-clear" id="{2}" class="image-clear-checkbox" style="position: absolute; opacity: 0; width: 1px; height: 1px; overflow: hidden;" onchange="toggleImageRemoval(this, \'{0}\', \'{1}\')">'
                    '  <label class="image-remove-btn" for="{2}" style="color: #dc3545; font-size: 0.9rem; font-weight: 600; cursor: pointer; display: none; align-items: center; margin-bottom: 12px; user-select: none;">'
                    '    <span class="remove-text" style="transition: all 0.2s ease;">Remove</span>'
                    '  </label>'
                    '  <label for="{4}" class="btn-change-image" style="background: var(--admin-gradient-primary); color: #ffffff; padding: 10px 20px; border-radius: var(--admin-radius-sm); font-weight: 600; font-size: 0.85rem; cursor: pointer; display: inline-block; transition: all 0.2s ease; box-shadow: 0 4px 12px var(--admin-accent-glow); text-transform: uppercase; margin-bottom: 8px;">'
                    '    Upload Image'
                    '  </label>',
                    img_id, placeholder_id, clear_checkbox_id, name, input_id
                )
            )
        ...
        html.append(
            format_html(
                ...
                '  <input type="file" name="{}" id="{}" style="position: absolute; opacity: 0; width: 1px; height: 1px; overflow: hidden;" class="image-file-input-hidden" onchange="previewSelectedImage(this, \'{}\', \'{}\', \'{}\')">'
                '</div>'
                '<script>'
                'function toggleImageRemoval(checkbox, imgId, placeholderId) {{'
                '    console.log("toggleImageRemoval called for checkbox:", checkbox.id, "checked:", checkbox.checked);'
                '    const img = document.getElementById(imgId);'
                '    const placeholder = document.getElementById(placeholderId);'
                '    const label = document.querySelector(\'label[for="\' + checkbox.id + \'"]\');'
                '    const text = label ? label.querySelector(\'.remove-text\') : null;'
                '    const fieldName = checkbox.name ? checkbox.name.replace("-clear", "") : "image";'
                '    const container = checkbox.closest(".view-image-card") || document.getElementById("preview_container_" + fieldName);'
                '    const fileInput = container ? container.querySelector("input[type=\'file\']") : null;'
                '    if (checkbox.checked) {{'
                '        if (img) img.style.display = "none";'
                '        if (placeholder) placeholder.style.display = "block";'
                '        if (text) {{'
                '            text.textContent = "Keep Image";'
                '            text.style.color = "var(--admin-accent-primary)";'
                '        }}'
                '        if (fileInput) {{'
                '            fileInput.value = "";'
                '            console.log("Cleared file input value due to image removal.");'
                '        }}'
                '    }} else {{'
                ...
                '    console.log("previewSelectedImage called. Input files:", input.files);'
                '    const img = document.getElementById(imgId);'
                '    const placeholder = document.getElementById(placeholderId);'
                ...
                '                img.style.display = "block";'
                '                img.style.opacity = "1";'
                '                console.log("Updated img src successfully.");'
                '            }}'
                '            if (placeholder) {{'
                ...
                '                toggleImageRemoval(checkbox, imgId, placeholderId);'
                '            }}'
                '            const container = input.closest(".view-image-card") || document.getElementById("preview_container_" + input.name);'
                '            const removeBtn = container ? container.querySelector(".image-remove-btn") : null;'
                '            if (removeBtn) {{'
                '                removeBtn.style.display = "inline-flex";'
                '                console.log("Displayed remove button.");'
                '            }}'
                '            const labelBtn = container ? container.querySelector(".btn-change-image") : null;'
                '            if (labelBtn) {{'
                '                labelBtn.textContent = "Change Image";'
                '                console.log("Updated button label text.");'
                '            }}'
                '        }};'
